...
def convert_html_to_pdf(html, output_file, stop_event):
    try:
        clean_html_content = clean_html(html)
        pdfkit.from_string(clean_html_content, output_file)
        if stop_event.is_set():
            print(f"线程接收到停止信号，终止处理: {output_file}")
            return
    except Exception as e:
...
def convert_html_to_pdf_in_thread(input_html_file, output_pdf_file):
    stop_event = threading.Event()
    thread = threading.Thread(target=convert_html_to_pdf, args=(input_html_file, output_pdf_file, stop_event))
    thread.start()
    return thread
...
def main(ebook_id, number, headers):
    ...
    for num in range(1, number + 1):
        output_file = f"{ebook_id}/{num}.pdf"
        ...
        try:
            ...
            stop_event = threading.Event()
            thread = threading.Thread(target=convert_html_to_pdf, args=(html, output_file, stop_event))
            thread.start()
            monitor = threading.Thread(target=monitor_thread, args=(thread, stop_event))
            monitor.start()
            threads.append((thread, monitor))
            print("PDF successfully created as", output_file)
        except Exception as e:
...
def get_book_content(ebook_id, book_name):
    ...
    # url = f"https://book.qq.com/book-read/32436084/146"
    # response = requests.get(url, headers=headers)
    # response.raise_for_status()
    output_file = f"{ebook_id}/0.pdf"
    html = fetch_html(url, headers)
    ...
    return book_name, number, html, output_file
...
if __name__ == "__main__":
    ...
    for ebook_id in book_list:
        ...
        try:
            os.mkdir(f'{ebook_id}')
            book_name, number, html, output_file = get_book_content(ebook_id, book_name)
            print(book_name)
            # print(number)
            # with open(f"{ebook_id}/0.html", 'w', encoding='utf-8') as file:
            #     file.write(html)
            convert_html_to_pdf_in_thread(html, output_file)
        except Exception:
            pass
